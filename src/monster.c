...
bool HEAD_flag(){
    ...
    // nuller(filename);
    // sprintf(filename,".monster/commits%d",id);
    // DIR* dir1=opendir(filename);
    // struct dirent* entry1;
    // DIR* dir2=opendir(".monster/commits");
...
int init(int argc,char* argv[]){
    ...
    if(!exists){
        ...
        if(mkdir(".monster/shortcuts", 0755) != 0) return 1;
...
int add_file(char* path){
    ...
    if(check_if_file_is_unstaged(path)=='U') remove(unstaged);
    fclose(file);
    fclose(file_to_add);
    return 0;
}
            ...
            if(strcmp(entry->d_name,".")!=0 && strcmp(entry->d_name,"..")!=0 && strcmp(entry->d_name,".monster")!=0 && isDirectory(entry->d_name)==0){
                ...
                while(fgets(address,sizeof(address),file)!=NULL){
                    ...
                    if(strcmp(filename,entry->d_name)==0){
                        if(check_if_file_is_unstaged(address)=='S') printf("STAGED\n");
...
void status_file(char* path){
    if(check_if_file_is_unstaged(path)=='S') printf("File %s status: +%c\n", path, status_mode(path));
...
int log_git(int argc,char* argv[]){
    ...
    if(argc==2){
        ...
        for(int i=3;i<argc;i++) show_some_log_plus(0,argv[i]);
        return 0;
    }
    else{
        printf("invalid inputs!\n");
...
void show_log(int counter,int number){
    if(counter==commitcounter()-number) return;
    char line[MAX_LINE_LENGTH];
    FILE* file=fopen(".monster/commitsinformation","r");
...
void show_some_log(int counter,char* argv[]){
    if(counter==commitcounter()) return;
    char line[MAX_LINE_LENGTH];
    FILE* file=fopen(".monster/commitsinformation","r");
    for(int i=counter;i<commitcounter()-1;i++){
        for(int j=0;j<8;j++){
    ...
    if(strcmp(argv[2],"-since")==0){     
        ...
        nuller(line);
        if((m_hour<hour) || (m_hour==hour&&m_min<min) || (m_hour==hour&&m_min==min&&m_sec<sec)){
            for(int i=0;i<8;i++){
                fgets(line,sizeof(line),file);
                line[strlen(line)-1]='\0';
                if(line[0]!='\0') puts(line);
...
int main (int argc,char* argv[]){
    ...
    if(strcmp(argv[1],"commit")==0) return commit(argc,argv);
    if((strcmp(argv[1],"set")==0 || strcmp(argv[1],"replace")==0) && strcmp(argv[2],"-m")==0 && strcmp(argv[4],"-s")==0) return set(argc,argv);
    if(strcmp(argv[1],"remove")==0 && strcmp(argv[2],"-s")==0) return removeshortcut(argc,argv);
    if(strcmp(argv[1],"log")==0) return log_git(argc,argv);
    if(strcmp(argv[1],"branch")==0) return branch(argc,argv);
    if(strcmp(argv[1],"checkout")==0) return checkout(argc,argv);
    if(init_flag()){
        char cwd[1024];
        ...
        while ((entry = readdir(dir)) != NULL) {
            if (strcmp(entry->d_name, argv[1]) == 0){
                ...
                if(strcmp(words[1],"status")==0) return status(count_of_words,words);
                if(strcmp(words[1],"commit")==0) return commit(count_of_words,words);
                if((strcmp(words[1],"set")==0 || strcmp(words[1],"replace")==0) && strcmp(words[2],"-m")==0 && strcmp(words[4],"-s")==0) return set(count_of_words,words);
                if(strcmp(words[1],"remove")==0 && strcmp(words[2],"-s")==0) return removeshortcut(count_of_words,words);
                if(strcmp(words[1],"log")==0) return log_git(count_of_words,words);
                if(strcmp(words[1],"branch")==0) return branch(count_of_words,words);
                if(strcmp(words[1],"checkout")==0) return checkout(count_of_words,words);
            }
        }
        closedir(dir);
